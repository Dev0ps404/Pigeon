...
const Settings = () => {
  ...
  const [sessions, setSessions] = useState([
    {
      ...
      client: "Pigeon Electron App v2.4.0",
      ip: "192.168.1.108",
    ...
    },
    {
      ...
      client: "Pigeon iOS Mobile v2.3.9",
      ip: "103.45.201.88",
    ...
    },
    {
      ...
      client: "Web Client Safari v17.4",
      ip: "172.56.90.112",
  ...
  const [aiSmartReply, setAiSmartReply] = useState(
    user?.aiSmartReply !== false,
  );
  const [aiSummarizer, setAiSummarizer] = useState(user?.aiSummarizer || false);
  const [aiToneEnhancer, setAiToneEnhancer] = useState(
    user?.aiToneEnhancer !== false,
  );
  const [aiPersonality, setAiPersonality] = useState(
    user?.aiPersonality || "concierge",
  );
  ...
  const playSynthSound = (soundType) => {
    try {
      ...
      if (soundType === "chime") {
        ...
        oscillator.frequency.setValueAtTime(1046.5, now + 0.16); // C6
        oscillator.start();
  ...
  const handleSaveAccount = async (e) => {
    ...
    try {
      ...
      toast.error(
        error.response?.data?.message || "Failed to save account settings",
      );
    } finally {
      setSaving(false);
    }
  ...
  const handleTogglePremium = async (val) => {
    ...
    try {
      ...
      dispatch(setCredentials({ user: data, token }));
      toast.success(
        val ? "Premium Star Shimmer Activated!" : "Premium mode disabled.",
      );
    } catch (error) {
  ...
  const handleSelectDisappearing = async (mode) => {
    ...
    try {
      ...
      dispatch(setCredentials({ user: data, token }));
      toast.success(
        `Disappearing messages set to: ${mode === "off" ? "Off" : mode}`,
      );
    } catch (err) {
  ...
  const handleToggleBlockUser = async (userId, userUsername) => {
    ...
    try {
      const { data } = await api.put("/auth/profile", {
        blockedContacts: updatedBlocks,
      });
      dispatch(setCredentials({ user: data, token }));
      toast.success(
        isAlreadyBlocked
          ? `Unblocked ${userUsername}`
          : `Blocked ${userUsername}`,
      );
    } catch (err) {
  ...
  const handleSendFeedback = (e) => {
    e.preventDefault();
    if (!feedbackMsg.trim())
      return toast.error("Feedback message cannot be empty");
  ...
  const renderAvatar = (size = "w-24 h-24", textClass = "text-3xl") => {
    ...
    const colorIndex =
      (username || user?.username || "U").charCodeAt(0) % colors.length;
    ...
    return (
      <div
        className={`${size} rounded-full bg-gradient-to-tr ${selectedGradient} text-white flex items-center justify-center font-black tracking-wider border-4 border-white dark:border-slate-900 shadow-sm ${textClass} select-none`}
      >
        {initial}
      </div>
    );
  ...
  const tabs = [
    {
      id: "account",
      label: "My Account",
      desc: "Profile details & avatar configuration",
      icon: FiUser,
    },
    {
      id: "privacy",
      label: "Privacy & Security",
      desc: "Read receipts, timer & blocked list",
      icon: FiShield,
    },
    {
      id: "notifications",
      label: "Notification Alerts",
      desc: "Ringtones, previews & synth testing",
      icon: FiBell,
    },
    {
      id: "storage",
      label: "Data & Storage",
      desc: "Telemetry usage metrics & limits",
      icon: FiActivity,
    },
    {
      id: "appearance",
      label: "Appearance",
      desc: "Custom themes, colors & layout scale",
      icon: FiMoon,
    },
    {
      id: "devices",
      label: "Connected Devices",
      desc: "Logged-in active sessions authorization",
      icon: FiSmartphone,
    },
    {
      id: "ai",
      label: "AI Preferences",
      desc: "AI smart reply & summaries controls",
      icon: FiCpu,
    },
    {
      id: "help",
      label: "Help & Support",
      desc: "FAQ accordions & developer tickets",
      icon: FiHelpCircle,
    },
  ];

  return (
    <div className="flex-1 flex h-full bg-[#f8fafc] dark:bg-[#070b13] overflow-hidden shadow-[-10px_0_30px_rgba(0,0,0,0.05)] border-t border-l border-white/50 dark:border-white/5 relative z-10">
      ...
      <div className="flex-1 flex flex-col h-full overflow-hidden w-full bg-white dark:bg-[#0b0f19]">
        ...
        <div className="flex-1 flex flex-col lg:flex-row overflow-hidden h-full w-full">
              ...
              {searchQuery ? (
                ...
                >
                  <div className="bg-primary-50/50 dark:bg-primary-950/20 border border-primary-100/60 dark:border-primary-900/30 p-4.5 rounded-[1.5rem] flex items-center gap-3.5 select-none">
                    ...
                    <div>
                      ...
                      <p className="text-[10.5px] text-primary-600/80 dark:text-primary-400/80 mt-0.5">
                        Showing matches matching your query across all
                        configuration cards
                      </p>
                    </div>
                  </div>
                  ...
                  {matchesSearch(
                    ...
                    "Username email phone number status bio profile",
                    "personal username phone email bio",
                  ) && (
                    <div className="bg-white dark:bg-[#0f172a] border border-gray-100 dark:border-slate-800/80 rounded-[2rem] p-6 shadow-sm">
                      ...
                      >
                        <div>
                          <label className="block text-[9.5px] font-bold text-gray-400 uppercase mb-1">
                            Username
                          </label>
                          <input
                            type="text"
                            ...
                            className="w-full text-xs px-3.5 py-2.5 rounded-xl border border-gray-150 dark:border-slate-800 bg-gray-50/40 dark:bg-slate-950/40 text-gray-900 dark:text-white"
                            required
                          />
                        </div>
                        <div>
                        ...
                        <div className="md:col-span-2">
                          <button
                            type="submit"
                            className="px-5 py-2.5 bg-primary-600 hover:bg-primary-700 text-white rounded-xl text-xs font-bold uppercase tracking-wider shadow-md shadow-primary-500/10"
                          >
                  ...
                  {/* 2. Privacy matches */}
                  {matchesSearch(
                    "Privacy Blocked Receipts Disappearing",
                    "Blocked contacts disappearing messages receipts",
                    "privacy blocked receipts disappearing contacts",
                  ) && (
                    <div className="bg-white dark:bg-[#0f172a] border border-gray-100 dark:border-slate-800/80 rounded-[2rem] p-6 shadow-sm space-y-4">
                      ...
                      <div className="flex justify-between items-center bg-gray-50/40 dark:bg-slate-950/20 p-3 rounded-xl">
                        ...
                        <button
                          onClick={() =>
                            handleToggleReadReceipts(!readReceipts)
                          }
                          className={`w-10 h-5.5 rounded-full relative transition-colors ${readReceipts ? "bg-primary-600" : "bg-gray-300 dark:bg-slate-700"}`}
                        >
                          <span
                            className={`w-3.5 h-3.5 bg-white rounded-full absolute top-1 transition-all ${readReceipts ? "left-[21px]" : "left-[3px]"}`}
                          />
                        </button>
                      </div>
                    </div>
                  ...
                  {matchesSearch(
                    "Notifications Alert Previews Sound Tones",
                    "Sound tones message group calls ringtones show previews",
                    "notifications alert tones sounds preview",
                  ) && (
                    <div className="bg-white dark:bg-[#0f172a] border border-gray-100 dark:border-slate-800/80 rounded-[2rem] p-6 shadow-sm space-y-4">
                      <div className="flex items-center gap-3 border-b border-gray-100 dark:border-slate-800/60 pb-3 mb-2">
                        <FiBell
                      ...
                      <div className="flex justify-between items-center bg-gray-50/40 dark:bg-slate-950/20 p-3 rounded-xl">
                        ...
                        <button
                          onClick={() =>
                            handleToggleShowPreviews(!showPreviews)
                          }
                          className={`w-10 h-5.5 rounded-full relative transition-colors ${showPreviews ? "bg-primary-600" : "bg-gray-300 dark:bg-slate-700"}`}
                        >
                          <span
                            className={`w-3.5 h-3.5 bg-white rounded-full absolute top-1 transition-all ${showPreviews ? "left-[21px]" : "left-[3px]"}`}
                          />
                        </button>
                      </div>
                    </div>
                  ...
                  {matchesSearch(
                    "Data & Storage Auto-Download Network Cellular",
                    "Telemetry session usage progress auto download photos video audio",
                    "data storage auto-download cellular network wifi",
                  ) && (
                    <div className="bg-white dark:bg-[#0f172a] border border-gray-100 dark:border-slate-800/80 rounded-[2rem] p-6 shadow-sm space-y-4">
                      <div className="flex items-center gap-3 border-b border-gray-100 dark:border-slate-800/60 pb-3 mb-2">
                        <FiActivity
                      ...
                      <p className="text-xs text-gray-550 dark:text-gray-400 leading-normal">
                        Search match: Check storage limits, telemetry bytes
                        transmitted, and cellular preferences inside the "Data &
                        Storage" navigation tab.
                      </p>
                    </div>
                  )}
                  ...
                  {/* 5. Help FAQ matches */}
                  {matchesSearch(
                    "Help FAQs FAQ Contact Us developer",
                    "Help FAQs expand Contact support ticket",
                    "help support contact faq guidelines policy",
                  ) && (
                    <div className="bg-white dark:bg-[#0f172a] border border-gray-100 dark:border-slate-800/80 rounded-[2rem] p-6 shadow-sm space-y-4">
                      <div className="flex items-center gap-3 border-b border-gray-100 dark:border-slate-800/60 pb-3 mb-2">
                        <FiSliders
                        ...
                        />
                        <h4 className="text-xs font-black text-gray-800 dark:text-white uppercase tracking-wider">
                          Help Accordion Support
                        </h4>
                      </div>
                      <p className="text-xs text-gray-550 dark:text-gray-400 leading-normal">
                        Search match: Contact developer, submit logs tickets,
                        and view terms FAQs. Go to "Help & FAQ Support" nav tab
                        for complete forms.
                      </p>
                    </div>
                  )}
                  ...
                  {matchesSearch(
                    "Appearance Theme Dark Light Cyberpunk Accent Colors Font Size",
                    "theme profile accent color font size typography scale",
                    "appearance themes darkmode colors fonts",
                  ) && (
                    <div className="bg-white dark:bg-[#0f172a] border border-gray-100 dark:border-slate-800/80 rounded-[2rem] p-6 shadow-sm space-y-4">
                      <div className="flex items-center gap-3 border-b border-gray-100 dark:border-slate-800/60 pb-3 mb-2">
                        <FiMoon
                      ...
                      <p className="text-xs text-gray-550 dark:text-gray-400 leading-normal">
                        Search match: Theme cards, accent highlights, and font
                        typography scale inside the "Appearance" tab.
                      </p>
                    </div>
                  )}
                  ...
                  {matchesSearch(
                    "Connected Devices Sessions Terminal Revoke",
                    "Active sessions devices revoke terminal safari browser iphone",
                    "connected devices sessions terminals revoke",
                  ) && (
                    <div className="bg-white dark:bg-[#0f172a] border border-gray-100 dark:border-slate-800/80 rounded-[2rem] p-6 shadow-sm space-y-4">
                      <div className="flex items-center gap-3 border-b border-gray-100 dark:border-slate-800/60 pb-3 mb-2">
                        <FiSmartphone
                      ...
                      <p className="text-xs text-gray-550 dark:text-gray-400 leading-normal">
                        Search match: Active terminal sessions, IP logs, and
                        session revoke inside the "Connected Devices" tab.
                      </p>
                    </div>
                  )}
                  ...
                  {matchesSearch(
                    "AI Preferences Smart Reply Summarizer Tone Persona",
                    "AI smart reply summarizer tone enhancer persona agent",
                    "ai preferences smart replies summaries tone",
                  ) && (
                    <div className="bg-white dark:bg-[#0f172a] border border-gray-100 dark:border-slate-800/80 rounded-[2rem] p-6 shadow-sm space-y-4">
                      <div className="flex items-center gap-3 border-b border-gray-100 dark:border-slate-800/60 pb-3 mb-2">
                        <FiCpu
                      ...
                      <p className="text-xs text-gray-550 dark:text-gray-400 leading-normal">
                        Search match: AI smart replies, chat summaries, tone
                        enhancer, and persona controls inside the "AI
                        Preferences" tab.
                      </p>
                    </div>
                  )}
                  ...
                  {!matchesSearch(
                    ...
                    !matchesSearch(
                      ...
                      <div className="text-center py-16 bg-white dark:bg-[#0f172a] border border-gray-100 dark:border-slate-800/80 rounded-[2.5rem] p-6 shadow-sm select-none">
                        ...
                        />
                        <h3 className="font-extrabold text-[15px] text-gray-850 dark:text-white">
                          No matches found
                        </h3>
                        <p className="text-xs text-gray-400 mt-1 max-w-[280px] mx-auto leading-relaxed">
                          We couldn't locate any settings configurations
                  ...
                  {activeTab === "account" && (
                    <div className="space-y-6">
                      ...
                      <div className="bg-white dark:bg-[#0f172a] border border-gray-100 dark:border-slate-850 rounded-[2.5rem] p-0 shadow-sm overflow-hidden space-y-6">
                        ...
                        <div className="relative">
                          ...
                          <div className="flex flex-col items-center -mt-16 pb-4 mb-2 space-y-2 relative">
                            ...
                            <div className="relative group cursor-pointer shrink-0 z-10">
                              <div
                                className={`p-1 rounded-full transition-transform duration-300 ${isPremium ? "premium-shimmer-border animate-pulse-slow shadow-lg shadow-amber-500/10" : "bg-white dark:bg-slate-900 border border-gray-200/50"}`}
                              >
                                {renderAvatar(
                                  "w-28 h-28",
                                  "text-4xl font-black",
                        ...
                        <div className="flex items-center gap-3 border-b border-gray-50 dark:border-slate-805 pb-4 px-6 lg:px-8">
                          ...
                          <div>
                            ...
                            <p className="text-[10px] text-gray-400 mt-0.5">
                              Modify username, phone, status alert, and bio
                              summaries
                            </p>
                          </div>
                        </div>
                        ...
                        <form
                          onSubmit={handleSaveAccount}
                          className="grid grid-cols-1 md:grid-cols-2 gap-5 px-6 lg:px-8 pb-6 lg:pb-8"
                        >
                          <div>
                          ...
                          <div>
                            ...
                            <div className="flex justify-between items-center px-4 py-2.5 rounded-xl border border-gray-150 dark:border-slate-800 bg-gray-50/40 dark:bg-slate-950/40 select-none">
                              <span className="text-xs font-semibold text-gray-650 dark:text-gray-300">
                              ...
                              >
                                <span
                                  className={`w-3.5 h-3.5 bg-white rounded-full absolute transition-all ${isPremium ? "left-[21px]" : "left-[3px]"}`}
                                />
                              </button>
                            </div>
                          </div>
                      ...
                      <div className="bg-white dark:bg-[#0f172a] border border-gray-100 dark:border-slate-850 rounded-[2.5rem] p-6 lg:p-8 shadow-sm space-y-6">
                        <div className="flex items-center gap-3 border-b border-gray-50 dark:border-slate-800/50 pb-4">
                          <FiLock
                            size={18}
                            className="text-[#1d70b8] dark:text-sky-400"
                          />
                          <div>
                            <h3 className="text-sm font-extrabold text-gray-800 dark:text-white leading-normal">
                            ...
                            <p className="text-[10px] text-gray-400 mt-0.5">
                              Maintain passwords for account access
                        ...
                        <div className="flex items-center gap-3.5">
                          ...
                          <div>
                            ...
                            <p className="text-[11px] text-gray-400 mt-0.5">
                              Toggle receipts visibility, disappearance timers
                              and restricted blocked lists
                            </p>
                          </div>
                        </div>
                      </div>
                            ...
                            <div className="flex justify-between items-start">
                              <div>
                                ...
                                <p className="text-[10px] text-gray-400 leading-normal mt-1 pr-4">
                                  Allow other members inside direct messaging
                                  channels to review when messages have been
                                  read
                                </p>
                              </div>
                              <button
                              ...
                              >
                                <span
                                  className={`w-4 h-4 bg-white rounded-full absolute transition-all ${readReceipts ? "left-[23px]" : "left-[4px]"}`}
                                />
                              </button>
                            </div>
                          </div>
                          ...
                          <div className="bg-white dark:bg-[#0f172a] border border-gray-100 dark:border-slate-855 p-6 rounded-[2.5rem] shadow-sm space-y-4">
                            <div>
                              ...
                              <p className="text-[10px] text-gray-405 leading-normal mt-1 pr-2">
                                Keep communication history clean by
                                automatically purging chat threads after set
                                periods
                              </p>
                            </div>
                        ...
                        <div className="bg-white dark:bg-[#0f172a] border border-gray-100 dark:border-slate-855 p-6 rounded-[2.5rem] shadow-sm flex flex-col h-full justify-between">
                          <div className="space-y-3.5">
                            <div className="flex justify-between items-center border-b border-gray-55 dark:border-slate-800/60 pb-3">
                              <div>
                                <h4 className="text-xs font-extrabold text-gray-800 dark:text-gray-200">
                                  Blocked Contacts Directory
                                </h4>
                                <p className="text-[10px] text-gray-450 mt-0.5">
                                  Mute direct chats & voice calls signaling
                                </p>
                              </div>
                              <button
                          ...
                          <div className="min-h-[170px] max-h-[220px] overflow-y-auto space-y-2 pr-1 custom-scrollbar flex-1 mt-4">
                            {isAddingBlock ? (
                              allUsers.length === 0 ? (
                                <p className="text-center text-[10px] text-gray-405 py-4">
                                  No other platform users listed.
                                </p>
                              ) : (
                                allUsers.map((item) => {
                                  ...
                                  return (
                                    <div
                                      key={item._id}
                                      className="flex justify-between items-center p-2.5 rounded-xl bg-gray-50/40 dark:bg-slate-950/20 border border-gray-100/50 dark:border-slate-850 text-xs"
                                    >
                              ...
                              <div className="text-center text-[10px] text-gray-400 py-10">
                                ...
                                />
                                <span>
                                  No restricted contacts are currently listed.
                                </span>
                              </div>
                            ) : (
                              blockedList.map((blockedId) => {
                                ...
                                const name = foundUser
                                  ? foundUser.username
                                  : `User ID: ${blockedId.slice(-6)}`;
                                return (
                                  <div
                                    key={blockedId}
                                    className="flex justify-between items-center p-3 rounded-xl bg-gray-50/40 dark:bg-slate-950/20 border border-gray-100/50 dark:border-slate-850 text-xs"
                                  >
                                    <span className="font-semibold text-gray-800 dark:text-slate-200">
                                      {name}
                                    </span>
                                    <button
                                      type="button"
                                      onClick={() =>
                                        handleToggleBlockUser(blockedId, name)
                                      }
                    ...
                    <div className="space-y-6">
                      ...
                      <div className="bg-white dark:bg-[#0f172a] border border-gray-100 dark:border-slate-855 p-6 rounded-[2.5rem] shadow-sm">
                        <div className="flex items-center gap-3.5">
                          ...
                          <div>
                            ...
                            <p className="text-[11px] text-gray-400 mt-0.5">
                              Modify preview banners visibility and locally
                              synthesize ringtone audio testers
                            </p>
                          </div>
                        </div>
                      </div>
                      ...
                      <div className="bg-white dark:bg-[#0f172a] border border-gray-100 dark:border-slate-855 p-6 rounded-[2.5rem] shadow-sm">
                        <div className="flex justify-between items-center">
                          <div>
                            <h4 className="text-xs font-extrabold text-gray-800 dark:text-gray-200">
                              Show Messaging Previews
                            </h4>
                            <p className="text-[10px] text-gray-405 leading-normal mt-1">
                              Enable floating push banners alerts on-screen with
                              message text body previews
                            </p>
                          </div>
                          <button
                            onClick={() =>
                          ...
                          >
                            <span
                              className={`w-4 h-4 bg-white rounded-full absolute transition-all ${showPreviews ? "left-[23px]" : "left-[4px]"}`}
                            />
                          </button>
                        </div>
                      </div>
                      ...
                      <div className="bg-white dark:bg-[#0f172a] border border-gray-100 dark:border-slate-855 p-6 rounded-[2.5rem] shadow-sm">
                        <div className="flex items-center gap-3.5">
                          ...
                          <div>
                            ...
                            <p className="text-[11px] text-gray-400 mt-0.5">
                              Track network byte telemetry indicators and
                              configure saving files policies
                            </p>
                          </div>
                        </div>
                      </div>
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                          ...
                          <div className="space-y-4 pt-1">
                            <div className="space-y-1.5">
                              <div className="flex justify-between text-xs font-bold text-gray-600 dark:text-slate-400">
                                ...
                                <span className="text-gray-850 dark:text-white">
                                  45.2 MB
                                </span>
                              </div>
                        ...
                        <div className="bg-white dark:bg-[#0f172a] border border-gray-100 dark:border-slate-855 p-6 rounded-[2.5rem] shadow-sm space-y-4">
                          ...
                          <div className="space-y-2.5 pt-1 select-none">
                            {[
                              {
                                title: "Photos over Wi-Fi network",
                                state: autoWifi,
                                setState: setAutoWifi,
                              },
                              {
                                title: "Photos over Cellular data limits",
                                state: autoCellular,
                                setState: setAutoCellular,
                              },
                              {
                                title: "Audio recordings & voicemails",
                                state: autoAudio,
                                setState: setAutoAudio,
                              },
                              {
                                title: "Large documents & PDF attachments",
                                state: autoDocs,
                                setState: setAutoDocs,
                              },
                            ].map((item, index) => (
                              <div
                                key={index}
                      ...
                      <div className="bg-white dark:bg-[#0f172a] border border-gray-100 dark:border-slate-855 p-6 rounded-[2.5rem] shadow-sm">
                        <div className="flex items-center gap-3.5">
                          ...
                          <div>
                            ...
                            <p className="text-[11px] text-gray-400 mt-0.5">
                              Customize your Pigeon aesthetic with themes,
                              accent colors, and typography scale
                            </p>
                          </div>
                        </div>
                      </div>
                        ...
                        <div className="bg-white dark:bg-[#0f172a] border border-gray-100 dark:border-slate-855 p-6 rounded-[2.5rem] shadow-sm space-y-4">
                          ...
                          <div className="grid grid-cols-1 gap-3.5">
                            ...
                            <motion.div
                              ...
                              onClick={() =>
                                handleSelectTheme("light", "Aura Light")
                              }
                            ...
                            >
                              <div className="flex items-center gap-3 relative z-10">
                                ...
                                <div className="text-left">
                                  <p className="text-xs font-bold text-gray-805 dark:text-white leading-none">
                                    Aura Light
                                  </p>
                                  <p className="text-[9.5px] text-gray-400 mt-1 font-medium">
                                    Clean white spaces, soft subtle gray borders
                                  </p>
                                </div>
                              </div>
                              {selectedTheme === "light" && (
                            ...
                            <motion.div
                              whileHover={{ scale: 1.01 }}
                              onClick={() =>
                                handleSelectTheme("dark", "Midnight Obsidian")
                              }
                              className={`relative p-4 rounded-2xl border-2 cursor-pointer flex items-center justify-between transition-all overflow-hidden ${
                                selectedTheme === "dark"
                                  ? "border-primary-500 bg-primary-50/20 dark:bg-primary-950/10"
                            ...
                            >
                              <div className="flex items-center gap-3 relative z-10">
                                ...
                                <div className="text-left">
                                  ...
                                  <p className="text-[9.5px] text-gray-400 mt-1 font-medium">
                                    Deep obsidian blue dark space, neon glows
                                  </p>
                                </div>
                              </div>
                              {selectedTheme === "dark" && (
                            ...
                            <motion.div
                              whileHover={{ scale: 1.01 }}
                              onClick={() =>
                                handleSelectTheme("cyberpunk", "Cyberpunk Dusk")
                              }
                              className={`relative p-4 rounded-2xl border-2 cursor-pointer flex items-center justify-between transition-all overflow-hidden ${
                            ...
                            >
                              ...
                              <div className="flex items-center gap-3 relative z-10">
                                ...
                                <div className="text-left">
                                  <p className="text-xs font-bold text-gray-855 dark:text-white leading-none">
                                    Cyberpunk Dusk
                                  </p>
                                  <p className="text-[9.5px] text-gray-400 mt-1 font-medium">
                                    Futuristic cyberpunk purple glass aesthetic
                                  </p>
                                </div>
                              </div>
                              {selectedTheme === "cyberpunk" && (
                        ...
                        <div className="space-y-6">
                          ...
                          <div className="bg-white dark:bg-[#0f172a] border border-gray-100 dark:border-slate-855 p-6 rounded-[2.5rem] shadow-sm space-y-4">
                            ...
                            <p className="text-[10px] text-gray-400 leading-normal pr-4">
                              Apply custom highlight colors across buttons and
                              badges
                            </p>
                              ...
                              {[
                                {
                                  id: "indigo",
                                  name: "Premium Indigo",
                                  class: "bg-indigo-600 shadow-indigo-500/20",
                                },
                                {
                                  id: "emerald",
                                  name: "Fresh Emerald",
                                  class: "bg-emerald-500 shadow-emerald-500/20",
                                },
                                {
                                  id: "rose",
                                  name: "Dynamic Rose",
                                  class: "bg-rose-500 shadow-rose-500/20",
                                },
                                {
                                  id: "amber",
                                  name: "Star Amber",
                                  class: "bg-amber-500 shadow-amber-500/20",
                                },
                              ].map((color) => (
                                <motion.button
                                  type="button"
                          ...
                          <div className="bg-white dark:bg-[#0f172a] border border-gray-100 dark:border-slate-855 p-6 rounded-[2.5rem] shadow-sm space-y-4">
                            ...
                            <div className="space-y-4 pt-1">
                              ...
                              <input
                                ...
                                onMouseUp={(e) =>
                                  handleSelectFontSize(parseInt(e.target.value))
                                }
                                onTouchEnd={(e) =>
                                  handleSelectFontSize(parseInt(e.target.value))
                                }
                              ...
                              <div className="p-4 rounded-2xl bg-gray-50/50 dark:bg-slate-950/30 border border-gray-100/50 dark:border-slate-850 select-none">
                                <p className="text-[9.5px] font-bold text-gray-455 uppercase tracking-widest mb-1.5">
                                  Live Preview
                                </p>
                                <p
                      ...
                      <div className="bg-white dark:bg-[#0f172a] border border-gray-100 dark:border-slate-855 p-6 rounded-[2.5rem] shadow-sm">
                        <div className="flex items-center gap-3.5">
                          ...
                          <div>
                            ...
                            <p className="text-[11px] text-gray-400 mt-0.5">
                              Manage and revoke authorized devices currently
                              signed into your Pigeon system
                            </p>
                          </div>
                        </div>
                      </div>
                      ...
                      <div className="bg-white dark:bg-[#0f172a] border border-gray-100 dark:border-slate-855 p-6 rounded-[2.5rem] shadow-sm space-y-5">
                        <div>
                          ...
                          <p className="text-[10px] text-gray-400 mt-1 leading-relaxed font-medium">
                            Authorized instances holding active decryption keys.
                            Inactive terminals can be disconnected.
                          </p>
                        </div>
                                ...
                                <div className="flex items-start gap-4">
                                  ...
                                  <div className="text-left space-y-1">
                                    <div className="flex items-center gap-2 flex-wrap">
                                      <h5 className="text-xs font-black text-gray-800 dark:text-white uppercase tracking-wider">
                                        {session.name}
                                      </h5>
                                      {session.current && (
                                    ...
                                    <p className="text-[10px] text-gray-550 dark:text-slate-400 font-semibold">
                                      {session.client}
                                    </p>
                                  ...
                                  {session.current ? (
                                    ...
                                    <motion.button
                                      ...
                                      onClick={() => {
                                        ...
                                        toast.success(
                                          `Access for ${session.name} revoked!`,
                                        );
                                      }}
                      ...
                      <div className="bg-white dark:bg-[#0f172a] border border-gray-100 dark:border-slate-855 p-6 rounded-[2.5rem] shadow-sm relative overflow-hidden">
                        ...
                        <div className="flex items-center gap-3.5 relative z-10">
                          ...
                          <div>
                            ...
                            <p className="text-[11px] text-gray-400 mt-0.5">
                              Configure ambient AI engines, conversation
                              summaries, and smart reply models
                            </p>
                          </div>
                        </div>
                      </div>
                          ...
                          <div className="space-y-3.5 pt-1">
                            ...
                            <div className="p-4 rounded-2xl bg-gray-50/40 dark:bg-slate-950/20 border border-gray-100/50 dark:border-slate-850 flex items-start justify-between gap-4">
                              <div className="text-left flex-1">
                                <h5 className="text-xs font-extrabold text-gray-805 dark:text-gray-250">
                                  AI Smart Replies
                                </h5>
                                <p className="text-[9.5px] text-gray-400 mt-1 leading-normal pr-4 font-medium">
                                  Auto-generate contextual smart response pills
                                  in conversations
                                </p>
                              </div>
                              <button
                                type="button"
                                onClick={() =>
                                  handleToggleAiSmartReply(!aiSmartReply)
                                }
                              ...
                              >
                                <span
                                  className={`w-4 h-4 bg-white rounded-full absolute transition-all ${aiSmartReply ? "left-[23px]" : "left-[4px]"}`}
                                />
                              </button>
                            </div>
                            ...
                            <div className="p-4 rounded-2xl bg-gray-50/40 dark:bg-slate-950/20 border border-gray-100/50 dark:border-slate-850 flex items-start justify-between gap-4">
                              <div className="text-left flex-1">
                                <h5 className="text-xs font-extrabold text-gray-805 dark:text-gray-250">
                                  Instant Chat Summaries
                                </h5>
                                <p className="text-[9.5px] text-gray-400 mt-1 leading-normal pr-4 font-medium">
                                  Extract unread channel highlights into concise
                                  bullet summaries
                                </p>
                              </div>
                              <button
                                type="button"
                                onClick={() =>
                                  handleToggleAiSummarizer(!aiSummarizer)
                                }
                              ...
                              >
                                <span
                                  className={`w-4 h-4 bg-white rounded-full absolute transition-all ${aiSummarizer ? "left-[23px]" : "left-[4px]"}`}
                                />
                              </button>
                            </div>
                            ...
                            <div className="p-4 rounded-2xl bg-gray-50/40 dark:bg-slate-950/20 border border-gray-100/50 dark:border-slate-850 flex items-start justify-between gap-4">
                              <div className="text-left flex-1">
                                <h5 className="text-xs font-extrabold text-gray-805 dark:text-gray-250">
                                  Writing Tone Enhancer
                                </h5>
                                <p className="text-[9.5px] text-gray-400 mt-1 leading-normal pr-4 font-medium">
                                  Analyze typing and suggest dynamic tone
                                  styling filters
                                </p>
                              </div>
                              <button
                                type="button"
                                onClick={() =>
                                  handleToggleAiToneEnhancer(!aiToneEnhancer)
                                }
                              ...
                              >
                                <span
                                  className={`w-4 h-4 bg-white rounded-full absolute transition-all ${aiToneEnhancer ? "left-[23px]" : "left-[4px]"}`}
                                />
                              </button>
                            </div>
                          </div>
                        ...
                        <div className="bg-white dark:bg-[#0f172a] border border-gray-100 dark:border-slate-855 p-6 rounded-[2.5rem] shadow-sm space-y-4 flex flex-col justify-between">
                          <div>
                            ...
                            <p className="text-[10px] text-gray-450 mt-1 leading-relaxed font-medium">
                              Determine the tone and personality style of your
                          ...
                          <div className="space-y-3 pt-1">
                            {[
                              {
                                id: "concierge",
                                title: "Sleek Concierge",
                                desc: "Neutral, professional, clean precision",
                              },
                              {
                                id: "nexus",
                                title: "Cyberpunk Nexus",
                                desc: "Sarcastic, high-tech, futuristic wits",
                              },
                              {
                                id: "zen",
                                title: "Zen Guru",
                                desc: "Calming, concise, minimalist guidance",
                              },
                            ].map((persona) => (
                              <motion.div
                                whileHover={{ scale: 1.015 }}
                                onClick={() =>
                                  handleSelectAiPersonality(
                                    persona.id,
                                    persona.title,
                                  )
                                }
                              ...
                              >
                                <div className="text-left relative z-10 flex-1">
                                  <h6 className="text-xs font-bold text-gray-855 dark:text-white leading-none">
                                    {persona.title}
                                  </h6>
                                ...
                                {aiPersonality === persona.id && (
                                  <FiCheck
                                    size={14}
                                    className="text-purple-500 z-10 shrink-0"
                                  />
                                )}
                              </motion.div>
                            ))}
                      ...
                      <div className="bg-white dark:bg-[#0f172a] border border-gray-100 dark:border-slate-855 p-6 rounded-[2.5rem] shadow-sm">
                        <div className="flex items-center gap-3.5">
                          ...
                          <div>
                            ...
                            <p className="text-[11px] text-gray-400 mt-0.5">
                              Clarify doubts through expandable FAQs list or
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                        ...
                        <div className="bg-white dark:bg-[#0f172a] border border-gray-100 dark:border-slate-855 p-6 rounded-[2.5rem] shadow-sm space-y-4">
                          ...
                          <div className="space-y-3 select-none">
                            {[
                              {
                                q: "Is Pigeon encryption standard secured?",
                                a: "Yes! All direct text chats and voice call signaling packets are fully cryptography protected using E2EE protocols.",
                              },
                              {
                                q: "How do I initiate an audio or video call?",
                                a: "Tap any friend or group header bar inside messaging, click the phone/video toggle icon, and wait for signaling modals.",
                              },
                              {
                                q: "What is Premium Member badge mode?",
                                a: "Premium accounts get special gold borders and animated sparkles on settings views and messaging tags to increase priority visual look.",
                              },
                            ].map((item, index) => {
                              ...
                              return (
                                <div
                                  key={index}
                                  className="border border-gray-100 dark:border-slate-800/80 rounded-2xl overflow-hidden bg-gray-50/20 dark:bg-slate-900/20 shadow-sm transition-all"
                                >
                                  <div
                                    onClick={() =>
                                      setFaqExpanded({
                        ...
                        <div className="bg-white dark:bg-[#0f172a] border border-gray-100 dark:border-slate-855 p-6 rounded-[2.5rem] shadow-sm flex flex-col h-full justify-between">
                          <div className="space-y-4">
                            <div>
                              ...
                              <p className="text-[10px] text-gray-450 mt-0.5">
                                Send crash logs or feature suggestions directly
                                to team
                              </p>
                            </div>
          ...
          <div className="hidden lg:flex flex-col w-76 shrink-0 bg-white dark:bg-[#090d16] border-l border-gray-150 dark:border-white/[0.04] p-5 overflow-y-auto custom-scrollbar space-y-2 select-none relative">
            ...
            <div className="rounded-2xl border border-slate-200/80 dark:border-white/[0.06] bg-slate-50/80 dark:bg-white/[0.03] p-4 shadow-sm space-y-3">
              <div className="flex items-start gap-3">
                <div className="w-11 h-11 rounded-2xl bg-emerald-500/15 text-emerald-600 dark:text-emerald-400 flex items-center justify-center">
                  <FiSettings size={18} className="stroke-[2.5]" />
                </div>
                <div className="min-w-0">
                  <div className="flex items-center gap-2">
                    ...
                    <span className="text-[10px] font-bold uppercase tracking-widest text-emerald-600/80 dark:text-emerald-400/80 bg-emerald-500/10 px-2 py-0.5 rounded-full">
                      v2.4.0
                    </span>
                  </div>
                  <p className="text-[11px] text-slate-500 dark:text-slate-400 mt-1 leading-relaxed">
                    Customize your Pigeon communication terminal credentials,
                    security, notifications, and ambient AI.
                  </p>
                </div>
              </div>
              ...
              <div className="relative">
                ...
                <input
                  ...
                  className="w-full pl-9 pr-9 py-2.5 rounded-xl border border-slate-200/80 dark:border-white/[0.06] bg-white dark:bg-white/[0.02] text-xs text-slate-700 dark:text-slate-200 placeholder-slate-400 focus:outline-none focus:ring-2 focus:ring-emerald-500/20"
                />
                {searchQuery && (
                  <button
                    type="button"
              ...
              <div className="flex items-center gap-3 rounded-xl border border-slate-200/70 dark:border-white/[0.06] bg-white/90 dark:bg-white/[0.02] p-3">
                <img
                  src={
                    user?.profilePicture ||
                  ...
                  alt="Profile"
                  referrerPolicy="no-referrer"
                  className="w-9 h-9 rounded-lg object-cover border border-slate-200/70 dark:border-white/[0.08]"
                />
                <div className="min-w-0">
            ...
            <div className="mt-6 pt-5 border-t border-gray-100 dark:border-white/[0.04] space-y-2.5">
              ...
              >
                <div className="shrink-0 w-9 h-9 flex items-center justify-center rounded-xl bg-gray-100 dark:bg-white/5 border border-gray-150 dark:border-white/[0.03] text-slate-455 dark:text-slate-500 group-hover:bg-gray-200 dark:group-hover:bg-white/10 group-hover:text-gray-800 dark:group-hover:text-white transition-all duration-300">
                  {selectedTheme === "dark" ? (
                    <FiSun size={16} className="stroke-[2.5]" />
                  ) : (
                    <FiMoon size={16} className="stroke-[2.5]" />
                  )}
                </div>
                <div className="flex-1 min-w-0">
                  <p className="text-[12.5px] font-bold text-gray-850 dark:text-white truncate leading-tight">
        ...
        {showLogoutModal && (
          <div className="fixed inset-0 bg-slate-950/65 backdrop-blur-sm z-50 flex items-center justify-center p-4">
            ...
            >
              ...
              <div>
                <h3 className="font-extrabold text-[16px] text-gray-900 dark:text-white tracking-tight">
                  Confirm Log Out
                </h3>
                <p className="text-xs text-gray-500 dark:text-gray-400 mt-2 leading-relaxed">
                  Are you absolutely sure you want to end your active
                  communication session on this device?
                </p>
              </div>
