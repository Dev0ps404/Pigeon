  ...
  return (
    <div className="flex-1 flex h-full bg-[#f3f4f6] dark:bg-[#030712] overflow-hidden rounded-tl-[2.5rem] shadow-[-10px_0_30px_rgba(0,0,0,0.05)] border-t border-l border-white/50 dark:border-white/5 relative z-10">
            ...
            groupChats.map((chat) => (
              ...
              >
                ...
                <div className="ml-4 flex-1 overflow-hidden">
                  <h3 className="font-semibold text-gray-900 dark:text-white truncate">{chat.chatName}</h3>
                  <p className="text-xs text-gray-500 truncate">{chat.users.length} members</p>
                </div>
              </div>
            ))
          )}
      ...
      <div className="flex-1 hidden md:flex flex-col items-center justify-center bg-gray-50/30 dark:bg-[#0a0f1c]">
        ...
        <h3 className="text-xl font-bold text-gray-800 dark:text-white">Group Messaging</h3>
        <p className="text-gray-500 text-sm max-w-xs text-center mt-2">Create groups to coordinate with multiple team members or friends at once.</p>
      </div>
      ...
      {isModalOpen && (
        <div className="fixed inset-0 bg-black/50 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          ...
          >
            <h3 className="text-xl font-bold text-gray-900 dark:text-white mb-4">Create New Group</h3>
            <form onSubmit={handleCreateGroup} className="space-y-4">
              ...
              <div>
                <label className="block text-sm font-semibold text-gray-700 dark:text-gray-300 mb-1">Add Members (Min 2)</label>
                <div className="max-h-48 overflow-y-auto space-y-2 border border-gray-100 dark:border-dark-border rounded-xl p-3 bg-gray-50 dark:bg-dark-bg custom-scrollbar">
                  {availableUsers.map((u) => (
                    <label key={u._id} className="flex items-center gap-3 cursor-pointer p-1.5 hover:bg-white dark:hover:bg-dark-card rounded-lg transition-colors">
                      <input 
                        type="checkbox"
                      ...
                      />
                      <img referrerPolicy="no-referrer" src={u.profilePicture} alt="avatar" className="w-8 h-8 rounded-full object-cover" />
